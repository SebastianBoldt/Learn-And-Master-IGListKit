//
//  Created by Sebastian Boldt on 06.03.18.
//  Copyright © 2018 Sebastian Boldt. All rights reserved.
//

import IGListKit

struct Constants {
    static let objects: [ListDiffable] = [SuperHero(firstName: "Peter", lastName: "Parker", superHeroName: "SpiderMan", icon: "🕷"),
                             Ad(description: "Want to know what happens in your future? 🔮?"),
                             SuperHero(firstName: "Bruce", lastName: "Wayne", superHeroName: "Batman", icon: "🦇"),
                             SuperHero(firstName: "Tony", lastName: "Stark", superHeroName: "Ironman", icon: "🤖"),
                             SuperHero(firstName: "Bruce", lastName: "Banner", superHeroName: "Incredible Hulk", icon: "🤢")]
}
